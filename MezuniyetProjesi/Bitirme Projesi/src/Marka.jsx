...
const Marka = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get('https://localhost:7223/api/Marka/List');
        setMarkaList(response.data);
      } catch (error) {
        console.error('Error fetching Marka list:', error);
      }
    };
  ...
  const handleEdit = (marka) => {
    // Düzenleme için seçili marka bilgilerini al ve modal'ı aç
  ...
  const handleDelete = (markaId) => {
    ...
    axios.delete(`https://localhost:7223/api/Marka/Delete?id=${markaId}`)
    .then(response => {
  ...
  const handleSaveEdit = () => {
    ...
    axios.put(`https://localhost:7223/api/Marka/Update`, selectedMarka)
      .then(() => {
  ...
  const handleAddNewMarka = () => {
    ...
    axios.post('https://localhost:7223/api/Marka/Insert', { ad: newMarka.ad })
      .then((response) => {
