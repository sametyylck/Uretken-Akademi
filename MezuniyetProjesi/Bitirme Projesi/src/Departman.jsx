...
const Departman = () => {
  ...
  useEffect(() => {
    // API'den Marka listesini al
    const fetchData = async () => {
      try {
        const response = await axios.get('https://localhost:7223/api/Departman/List');
        setMarkaList(response.data);
      } catch (error) {
        console.error('Error fetching Departman list:', error);
      }
    };
  ...
  const handleEdit = (marka) => {
    // Düzenleme için seçili Departman bilgilerini al ve modal'ı aç
  ...
  const handleDelete = (markaId) => {
    ...
    axios.delete(`https://localhost:7223/api/Departman/Delete?id=${markaId}`)
         .then(response => {
  ...
  const handleSaveEdit = () => {
    ...
    axios.put(`https://localhost:7223/api/Departman/Update`, selectedMarka)
      .then(() => {
      ...
      .catch((error) => {
        console.error('Error updating Departman:', error);
      });
  };

  const handleAddNewMarka = () => {
    // Yeni marka ekleme işlemi için API'ye yeni marka bilgilerini gönder
    axios.post('https://localhost:7223/api/Departman/Insert', { ad: newMarka.ad })
      .then((response) => {
      ...
      .catch((error) => {
        console.error('Error adding new Marka:', error);
      });
  };
